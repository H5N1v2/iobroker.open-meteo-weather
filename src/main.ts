...
class OpenMeteoWeather extends utils.Adapter {
    ...
    private getWindGustIcon(gusts: number): string {
        ...
        if (gusts < 39 / factor) return ''; 
        if (gusts < 50 / factor) return `/adapter/${this.name}/icons/wind_icons/0.png`;
        if (gusts < 62 / factor) return `/adapter/${this.name}/icons/wind_icons/1.png`;
        if (gusts < 75 / factor) return `/adapter/${this.name}/icons/wind_icons/2.png`;
        if (gusts < 89 / factor) return `/adapter/${this.name}/icons/wind_icons/3.png`;
    ...
    private async cleanupObjects(): Promise<void> {
        ...
        try {
            ...
            const maxDays = parseInt(config.forecastDays as any) || 0;
            const maxHoursLimit = parseInt(config.forecastHours as any) || 0;
    ...
    private async updateData(): Promise<void> {
        try {
            ...
            if (data.weather) await this.processWeatherData(data.weather);
            if (data.hourly) await this.processForecastHoursData(data.hourly);
            if (data.air) await this.processAirQualityData(data.air);
